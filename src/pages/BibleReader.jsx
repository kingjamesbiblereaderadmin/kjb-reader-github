...
export default function BibleReader() {
  ...
  return (
    <div onClick={(e) => { if (!e.target.closest('.kjb-verse-container, [data-audio-verse], h1, h2, h3, .kjb-subscript, .kjb-colophon, #kjb-colophon-anchor, #kjb-subscript-anchor, button, a, [role="menu"], [role="menuitem"], [data-radix-popper-content-wrapper]')) { setHighlightVerse(null); setHighlightSection(null); setTappedVerses(new Set()); if (!selectMode) setHighlightedVerses(new Set()); } }} className={`w-full max-w-[120rem] mx-auto px-5 sm:px-8 lg:px-12 py-3 ${hideHeader ? 'pt-16' : ''}`}>
      {!hideHeader && (
      ...
      >
        ...
        {!loading && !error && verses.length > 0 && (() => {
          ...
          return (
          <div ref={useColumns ? columnsContainerRef : null} className={`${useColumns ? `kjb-two-col text-left ${fontFamily === 'cursive' ? '' : 'hyphens-auto'}` : 'text-left'} ${paragraphMode ? 'text-left px-2 sm:px-4' : ''}`} style={useColumns ? { fontSize: 'inherit', columnCount: 2, columnGap: fontFamily === 'cursive' ? '3.5rem' : '2.5rem', columnRule: '1px solid hsl(var(--border))', ...(fontFamily === 'cursive' ? { hyphens: 'none', WebkitHyphens: 'none' } : {}) } : { fontSize: 'inherit' }}>
            {subscriptBlock}
