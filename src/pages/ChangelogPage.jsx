...
const CHANGELOG = [
  { version: 'v0.4.183', date: 'August 19, 2026', items: ['Fixed the reading area becoming invisible and unscrollable on short viewports (mobile landscape and small popup windows). The panel now scrolls as a single document instead of a clipped full-height column, so the reading area can no longer collapse to nothing.', 'The legal footer is now pinned to the bottom edge and stays visible at every scroll position, including inside the in-page overlay where it was previously missing.', 'The in-page overlay now sizes itself to the visible viewport, so its lower portion is no longer hidden behind mobile browser toolbars.', 'Verified across interface zoom levels from 75% to 150% in both portrait and landscape.'] },
